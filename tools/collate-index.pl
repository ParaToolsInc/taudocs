# -*- Perl -*-
#
# $Id: collate-index.pl,v 1.1 2005/06/17 22:34:09 hoge Exp $

# This program generates an index automatically from DocBook code. It
# reads the existing HTML.index file, and updates the index.sgml file.
# If any changes have been made to the SGML source code that affect
# pagination of the HTML or print output, this program must be run twice.  

# This is a modified version of Norman Walsh's collateindex.pl.  The
# modification was made to allow the program to differentiate between
# index term scope of 'all', 'local', or 'global', and to generate
# indices for books or sets, based on those parameters.  Modifications
# by Sam Roberts and Bob McIlvride.
...
Where <opts> are:
       ...
       -B book   The generated index will include only IndexTerms from this
                 book. If unspecified, the default is not to filter based
                 on the book.
       -x        Make a SetIndex.
...
die $usage if ! getopts('Dfgi:NpP:s:o:S:I:B:t:x');
...
D.2. collate-all         D.4. docs2andrewgate.sh
Copyright © 1995-2004 by Cogent Real-Time Systems, Inc. All rights
reserved.Preparing Cogent Documentation : 
    Appendix D. Scripts in doc/bin/  
D.3. collateindex.pl

    
The collateindex file (with no .pl extension) is a shell script that executes
this Perl program.
